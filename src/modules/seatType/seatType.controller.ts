...
export const createSeatType = catchAsync(async (req: Request, res: Response) => {
    ...
    const result = await seatTypeService.createSeatType(req.validated!.body, req.user._id.toString());
...
export const updateSeatType = catchAsync(async (req: Request, res: Response) => {
    if (!req.user) throw new AppError("Vui lòng đăng nhập", 401);
    const result = await seatTypeService.updateSeatType(req.params.id, req.validated!.body, req.user._id.toString());
