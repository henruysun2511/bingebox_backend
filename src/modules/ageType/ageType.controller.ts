...
export const getAgeTypes = catchAsync(async (req: Request, res: Response) => {
    const result = await service.getAgeTypes(req.validated!.query);
...
export const createAgeType = catchAsync(async (req: Request, res: Response) => {
    ...
    const result = await service.createAgeType(req.validated!.body, req.user._id.toString());
    return success(res, result, "Tạo thành công", 201);
});

export const updateAgeType = catchAsync(async (req: Request, res: Response) => {
    if (!req.user) throw new AppError("Vui lòng đăng nhập", 401);
    const result = await service.updateAgeType(req.params.id, req.validated!.body, req.user._id.toString());
